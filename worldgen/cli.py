...
from . import (clutter as clutter_mod, course as course_mod, evaluate,
               geometry, maze as maze_mod, presets, preview, sdf)
...
def _cmd_maze(args):
    """Generate a grid maze world + its left-hand-rule course sidecar."""
    openings = maze_mod.carve(args.cols, args.rows, args.seed)
    walls = maze_mod.maze_walls(args.cols, args.rows, openings, args.cell)
    path_pts = maze_mod.left_hand_path(args.cols, args.rows, openings, args.cell)
    name = args.name or f"gen_maze_{args.cols}x{args.rows}_s{args.seed}"
    generator = {"maze": True, "cols": args.cols, "rows": args.rows,
                 "cell": args.cell, "seed": args.seed}
    course_data = course_mod.course_dict(
        name, path_pts, args.cell / 2.0, True, False, False, generator)
    stamp = datetime.date.today().isoformat()
    comment = (f"generated by worldgen maze on {stamp}: {args.cols}x{args.rows} "
               f"cell={args.cell} seed={args.seed}")
    out_dir = args.out or _default_worlds_dir()
    _write_world(walls, name, out_dir, comment, course_data=course_data,
                 show_preview=not args.no_preview, dry_run=args.dry_run)
    return 0
...
def main(argv=None):
    ...
    mz = sub.add_parser("maze",
                        help="generate a grid maze (solvable by left-wall "
                             "following)")
    mz.add_argument("--cols", type=int, default=4, help="maze columns (default 4)")
    mz.add_argument("--rows", type=int, default=4, help="maze rows (default 4)")
    mz.add_argument("--cell", type=float, default=3.0,
                    help="corridor pitch in metres; clear width = cell - 0.2 "
                         "(default 3.0)")
    mz.add_argument("--seed", type=int, default=0, help="maze seed (default 0)")
    mz.add_argument("--name", default=None, help="output file name (no .world)")
    mz.add_argument("--out", type=Path, default=None,
                    help="output directory (default: the package worlds/ dir)")
    mz.add_argument("--no-preview", action="store_true")
